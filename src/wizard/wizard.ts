...
/**
 * Multi-step wizard with back-button support.
 * cf. https://github.com/microsoft/vscode-azuretools (AzureWizard)
 */
export class Wizard<T extends WizardContext> {
    ...
    private readonly _wizardHideStepCount?: boolean;
    ...
    public get totalSteps(): number {
        return this._finishedPromptSteps.filter(s => s.prompted).length
            + this._promptSteps.filter(s => s.shouldPrompt(this._context)).length + 1;
    }
    ...
    public async prompt(): Promise<void> {
        ...
        while (step) {
            ...
            if (step.getSubWizard) {
                const subWizard = await step.getSubWizard(this._context);
                if (subWizard) {
    ...
    private addSubWizard(step: WizardPromptStep<T>, subWizard: IWizardOptions<T>): void {
        ...
        if (subWizard.promptSteps) {
            subWizard.promptSteps = subWizard.promptSteps.filter(s1 =>
                ...
                || !this._finishedPromptSteps.concat(this._promptSteps)
                    .some(s2 => getEffectiveStepId(s1) === getEffectiveStepId(s2)));
...
/**
 * Runs the wizard and returns the collected context,
